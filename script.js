...
// Movement Physics (Fast Response)
const LERP_SPEED = 0.6; // High value for instant reaction
...
function startGame(reset) {
    ...
    // Clear World
    obstacles.forEach(o => scene.remove(o)); trees.forEach(t => scene.remove(t)); 
    obstacles = []; trees = [];
...
function spawnObstacle() {
    ...
    if (type === 0) { // JUMP (Low)
        geo = new THREE.BoxGeometry(2.5, 0.4, 0.4); 
        mat = new THREE.MeshLambertMaterial({ color: 0xFFA500 }); // Orange
        yPos = 0.2; col = 'jump';
    } 
    else if (type === 1) { // DUCK (High)
...
function triggerSlide() {
    ...
    // 0.8 সেকেন্ড পর অটোমেটিক দাঁড়িয়ে যাবে
...
function animate() {
    ...
    for(let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (ob.position.z > -1 && ob.position.z < 1 && Math.abs(ob.position.x - player.position.x) < 1) {
            let hit = false;
            if (ob.userData.type === 'jump' && player.position.y < 0.6) hit = true;
            if (ob.userData.type === 'slide' && (!isSliding && player.position.y > 1.5)) hit = true;
            if (ob.userData.type === 'dodge') hit = true;
    ...
    trees.forEach((t, i) => { t.position.z += gameSpeed; if (t.position.z > 10) { scene.remove(t); trees.splice(i, 1); } });
...
function setupControls() {
    ...
    document.addEventListener('keydown', e => {
        ...
        if(e.key === 'ArrowDown') triggerSlide(); // PC: Down Arrow for slide
    });
    ...
    document.addEventListener('touchend', e => {
        ...
        if (tapLength < 300 && tapLength > 0) {
            triggerSlide(); // Double tap detected!
            e.preventDefault();
        }
        ...
        if(Math.abs(dx) > Math.abs(dy)) {
            ...
            // Vertical Swipe (Only Up for Jump)
            if(dy < -30 && !isJumping) {
...
function gameOver() { 
    ...
    clearTimeout(spawnTimeout); cancelAnimationFrame(animationId); 
    document.getElementById('hud').classList.add('hidden'); 
    document.getElementById('final-score').innerText = score; 
    document.getElementById('game-over-screen').classList.remove('hidden'); 
    adCount = 0; updateAdButtons(); 
}

function watchAd() { 
    const b = document.getElementById('btn-watch-ad'); b.disabled = true; b.innerText = "LOADING..."; 
    if(typeof show_10416947 === 'function') {
        show_10416947().then(() => { adCount++; updateAdButtons(); }).catch(() => { b.innerText = "FAILED"; b.disabled = false; }); 
    } else { setTimeout(() => { adCount++; updateAdButtons(); }, 1000); }
}
...
function updateAdButtons() { 
    ...
    if(adCount < ADS_REQUIRED) { 
        b.style.display = 'inline-block'; r.style.display = 'none'; 
        b.innerText = `WATCH AD (${adCount}/${ADS_REQUIRED})`; b.disabled = false; 
    } else { 
        b.style.display = 'none'; r.style.display = 'inline-block'; 
    } 
}
